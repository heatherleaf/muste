...
class Parser(object):
    ...
    def nbest_parse(self, tokens, n=None, trace=None):
        """
        Parse a sequence of tokens, returns a list of pairs
        (weight, parse tree). The weight is the negative logarithmic 
        probability, use exp(-weight) to the probability of the tree.

        The trees are returned in increasing weight order.
        Currently the parsing is brute force: calculate ALL parse
        trees, and sort them in weight order.
        ...
        """
        self.chart_parse(tokens, trace)
        weighted_trees = [(treeweight(tree, self.grammar['weights']), tree)
                           for tree in self.extract_trees()]
        weighted_trees.sort()
        if n: weighted_trees = weighted_trees[:n]
...
def treeweight(tree, weights):
    if isinstance(tree, (tuple, list)):
        return sum(treeweight(t, weights) for t in tree)
    else:
        return weights[tree]
...
######################################################################
## parsing PROBABILISTIC grammars with no epsilon rules
## Angelov's weighted algoeithm, initial attempt 

import heapq

def probabilistic_chart_parse(tokens, grammar, filtered=False):
    positions = range(1 + len(tokens))
    sequences = grammar['sequences']
    grammar_weights = grammar['weights']
    chart = {}
    chart_weights = chart['weights'] = {}
    active_chart = chart[Active] = [defaultdict(set) for _ in positions]
    found_chart = chart[Found] = [[set() for _ in positions] for _ in positions]
    dynrule_chart = chart[Rule] = defaultdict(set)
    predict_chart = chart[Symbol] = [defaultdict(set) for _ in positions]
    agenda = []
    starttime = time.clock()
    ...
    for startcat in grammar['startcats']:
        for lbl in grammar['catlabels'][startcat]:
            predict = (0, Symbol(startcat, lbl))
            w_in = grammar_weights.get(startcat, 0)
            add_edge((w_in, w_in, predict), predict_chart[0][startcat], "init TD")
            final_edges.add(Found(startcat, lbl, 0, len(tokens)))

    # process agenda
    while agenda:
        (w_inout, w_in, edge) = heapq.heappop(agenda)
        print w_inout, w_in, edge
        ...
        if type(edge) is Active:
            ...
            if active.nextsym is None:
                # complete
                found = Found(rule.cat, active.lbl, active.start, active.end)
                newrule = Rule(rule.fun, found, rule.args)
                chart_weights[found] = w_in
                add_edge((0, 0, found), found_chart[active.end][active.start], "complete", active)
                add_edge((0, 0, newrule), dynrule_chart[found], "complete", active)

            elif type(active.nextsym) is Symbol:
                # predict item
                predict = (active.end, active.nextsym)
                add_edge((w_inout, w_in, predict), predict_chart[active.end][active.nextsym.cat], "pred. item", active)
                # combine
                found = Found(active.nextsym.cat, active.nextsym.lbl, active.end, active.end)
                if found in found_chart[active.end][active.end]:
                    combine_inference_rule(active, found, sequences, active_chart, add_edge, "combine", active, found)

            elif active.end < len(tokens) and active.nextsym == tokens[active.end]:
                # scan
                probabilistic_add_active_edge(
                    w_inout, w_in, 
                    active.start, active.end+1, active.lbl, active.pos+1, active.rule,
                    sequences, active_chart, add_edge, "scan", active
                    )
        ...
        elif type(edge) is tuple and type(edge[1]) is Symbol:
            k, predict = edge
            cat = predict.cat
            if type(cat) is Found:
                # predict next
                for rule in dynrule_chart[cat]:
                    assert cat == rule.cat, (cat, rule.cat)
                    w2_in = grammar_weights[rule.fun] 
                    w2_in += sum(chart_weights[arg] if arg in chart_weights 
                                 else grammar_weights.get(arg, 0)
                                 for arg in rule.args)
                    w2_inout = w2_in + w_inout
                    w2_inout -= (chart_weights[cat] if cat in chart_weights 
                                 else grammar_weights.get(cat, 0))
                    probabilistic_add_active_edge(
                        w2_inout, w2_in,
                        k, k, predict.lbl, 0, rule, sequences,
                        active_chart, add_edge, "pred. next", rule, predict
                        )
                ...
                for rule in grammar['topdown'][cat]:
                    ...
                    probabilistic_add_active_edge(
                        w2_inout, w2_in,
                        k, k, predict.lbl, 0, rule, sequences,
                        active_chart, add_edge, "pred. TD", predict
                        )
            ...
            nextsym = Symbol(found.cat, found.lbl)
            # combine
            for (w2_inout, w2_in, active) in active_chart[start][nextsym]:
                w2_diff = chart_weights[found] - grammar_weights.get(found.cat, 0)
                probabilistic_combine_inference_rule(
                    w2_inout + w2_diff, w2_in + w2_diff,
                    active, found, sequences, active_chart,
                    add_edge, "combine", active, found
                    )
    ...
    statistics = {}
    statistics['Time'] = time.clock() - starttime
    statistics['Chart'] = dict((key, objsize(value)) for (key, value) in chart.items())
    return parse_result, chart, statistics


######################################################################
## PROBABILISTIC inference rules for parsing

def probabilistic_combine_inference_rule(w1, w2, active_edge, found, sequences,
                                         active_chart, add_edge, name, *antecedents):
    assert type(active_edge) == Active, (type(active_edge), active_edge)
    newrule = update_rule(active_edge.rule, active_edge.lbl, active_edge.pos, found, sequences)
    probabilistic_add_active_edge(
        w1, w2,
        active_edge.start, found.end, active_edge.lbl, active_edge.pos+1,
        newrule, sequences, active_chart, add_edge, name, *antecedents
        )
    
def probabilistic_add_active_edge(w1, w2, i, k, lbl, p, rule, sequences,
                                  active_chart, add_edge, name, *antecedents):
    assert type(rule) == Rule, (type(rule), rule)
    rhs = sequences[rule.fun][lbl]
    if p < len(rhs):
    ...
    if type(nextsym) is RHSSymbol:
        nextsym = nextsym.toSymbol(rule.args)
    newedge = (w1, w2, Active(i, k, lbl, nextsym, p, rule))
    add_edge(newedge, active_chart[k][nextsym], name, *antecedents)
