    ...
    def _linearise_child_nondet(self, parent, child, path, i):
        if child is None:
            ...
            for cat in self.cnccats[childtype]:
                if cat in self.arities:
                    lins = tuple([([META], [], append_path(path, i))] 
                                 for _ in xrange(self.arities[cat]))
                    yield cat, lins
        else:
...
def append_path(path, n):
    return concat_path(path, (n,))

def prepend_path(n, path):
    return concat_path((n,), path)

def concat_path(prefix, suffix):
    try:
...
def str_lin(lin, *args, **kwargs):
    if isinstance(lin, (tuple,list)) and all(isinstance(l, list) for l in lin):
        return " ; ".join(str_lin(phrase, *args, **kwargs) for phrase in lin)
    else:
        return " ".join(str_token(token, *args, **kwargs) for token in lin) 

def str_token(token, marked=None, exact=False, showpath=False):
    if isinstance(token, basestring):
        ...
        result = ""
        if is_path(marked):
            if (path == marked if exact else startswith(path, marked)):
                result += "*"
        result += word
        if showpath:
            result += "/" + str_path(path)
        return result
